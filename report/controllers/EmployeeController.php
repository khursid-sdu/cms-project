...
class EmployeeController extends Controller
{
    ...
    public function actionEmpinforeport()
    {
        ...
        $newvar = Yii::$app->request->post('e_info');
        if(!empty($newvar))
        {
 ...
 // 1. Only show employees who actually have published papers
            if (in_array('emp_paperpublished', $selected_list)) {
                $employee_d->andWhere(['and', 
                    ['is not', 'e_info.emp_paperpublished', null], 
                    ['<>', 'e_info.emp_paperpublished', '']
                ]);
            }
            // 2. Sort Active (status_id = 1) A-Z FIRST, then Inactive (status_id != 1) A-Z
            $employee_d->orderBy([
                new \yii\db\Expression('(CASE WHEN emp.emp_master_status_id = 1 THEN 0 ELSE 1 END) ASC'),
                'e_info.emp_first_name' => SORT_ASC,
                'e_info.emp_last_name' => SORT_ASC,
            ]);
